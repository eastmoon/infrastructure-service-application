...
+ 配置對象的授權資訊 ```[module_name].authorize```
		- 若配置對象的基礎服務有提供相應操作，應基於該關鍵字處理。

+ 配置對象的安全性資訊 ```[module_name].secure```
		- 若配置對象的基礎服務有提供相應操作，應基於該關鍵字處理。

+ 配置對象的操作程序 ```[module_name].command```
...
+ 通用設計規範
		...
		- 模組關鍵字執行順序：
				+ 模組固定項目：容器資訊、授權資訊、安全性資訊、操作程序
						- 若無關鍵字則忽略該動作
				+ 上述關鍵字外，依關鍵字順序比對模組是否提供對應處理函數，若存在則執行
